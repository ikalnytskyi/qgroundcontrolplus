...
void CustomPlugin::createRootWindow(QQmlApplicationEngine *qmlEngine)
{
    ...
    _installStartupOverlay(qmlEngine);
}

void CustomPlugin::_installStartupOverlay(QQmlApplicationEngine *qmlEngine)
{
#ifdef Q_OS_ANDROID
    if (!qmlEngine) {
        return;
    ...
    QTimer::singleShot(0, qmlEngine, [qmlEngine]() {
        if (qmlEngine->rootObjects().isEmpty()) {
            qCWarning(CustomLog) << "Startup splash skipped: no root objects";
            return;
        }

        QQuickWindow *const window = qobject_cast<QQuickWindow *>(qmlEngine->rootObjects().constFirst());
        if (!window || !window->contentItem()) {
            qCWarning(CustomLog) << "Startup splash skipped: root window unavailable";
            return;
        }

        // Avoid the white startup surface while the first QML scene settles.
        window->setColor(Qt::black);

        QQmlComponent component(qmlEngine, QUrl(QStringLiteral("qrc:/Custom/qml/QGroundControl/CustomStartupSplash.qml")));
        if (component.status() != QQmlComponent::Ready) {
            qCWarning(CustomLog) << "Failed to load startup splash component:" << component.errors();
            return;
        }

        QObject *const createdObject = component.create(qmlEngine->rootContext());
        QQuickItem *const overlayItem = qobject_cast<QQuickItem *>(createdObject);
        if (!overlayItem) {
            qCWarning(CustomLog) << "Startup splash did not create a QQuickItem";
            delete createdObject;
            return;
        }

        overlayItem->setParent(window->contentItem());
        overlayItem->setParentItem(window->contentItem());
        qCDebug(CustomLog) << "Installed custom startup splash overlay";
    });
#else
    Q_UNUSED(qmlEngine);
#endif
