...
DECLARE_SETTINGSFACT_NO_FUNC(Video2Settings, lowLatencyMode)
{
    if (!_lowLatencyModeFact) {
        ...
        connect(_lowLatencyModeFact, &Fact::valueChanged, this, &Video2Settings::_configChanged);
    }
    return _lowLatencyModeFact;
}

DECLARE_SETTINGSFACT_NO_FUNC(Video2Settings, forceCpuVideoPath)
{
...
DECLARE_SETTINGSFACT_NO_FUNC(Video2Settings, videoConversionElement)
{
...
DECLARE_SETTINGSFACT_NO_FUNC(Video2Settings, disablePixelAspectRatio)
{
...
DECLARE_SETTINGSFACT_NO_FUNC(Video2Settings, frameSmoothingEnabled)
{
...
DECLARE_SETTINGSFACT_NO_FUNC(Video2Settings, rtspTimeout)
{
    if (!_rtspTimeoutFact) {
        ...
        connect(_rtspTimeoutFact, &Fact::valueChanged, this, &Video2Settings::_configChanged);
    }
...
bool Video2Settings::streamConfigured(void)
{
    ...
    if(VideoManager::instance()->autoStreamConfigured()) {
        qCDebug(Video2SettingsLog) << "Stream auto configured";
        return true;
    }
    ...
    if (UVCReceiver::enabled() && UVCReceiver::deviceExists(vSource)) {
        qCDebug(Video2SettingsLog) << "Stream configured for UVC";
        return true;
    }
#endif
    return false;
...
void Video2Settings::_setForceVideoDecodeList()
{
#ifdef QGC_GST_STREAMING
