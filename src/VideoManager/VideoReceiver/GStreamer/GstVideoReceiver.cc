...
void _setNullableStringProperty(GstElement *element, const char *propertyName, const char *value)
{
    ...
    g_value_set_string(&propertyValue, value);
    g_object_set_property(G_OBJECT(element), propertyName, &propertyValue);
...
GstElement *GstVideoReceiver::_makeSource(const QString &input)
{
    ...
    const bool isWhep = sourceUrl.scheme().startsWith("http", Qt::CaseInsensitive);
    ...
    do {
        if (isRtsp) {
            ...
            if (!GStreamer::isValidWhepUri(input.toUtf8().constData())) {
                qCCritical(GstVideoReceiverLog) << "Invalid WHEP URI:" << input;
                break;
            }
